(function () {
  ...
  // v12.29：线上连续点击确认 C01、C03、C08 的镜头前冲会同时放大猫身并漂移脸型。
  // 三段源视频继续留在 video-v2 供定点返修；当前运行时先退回整猫像素动作，不用闪烁掩盖结构变化。
  var videoActions = {
    ...
    peek: { id: "C05", duration: 5042, startBox: [802, 296, 263, 328], transitionMs: 440 },
  ...
  function stopPetVideo() {
    ...
    window.clearTimeout(videoTailTimer);
  ...
  function containedMediaRect(containerRect, sourceWidth, sourceHeight) {
    var scale = Math.min(containerRect.width / sourceWidth, containerRect.height / sourceHeight);
  ...
  function alignPetVideo(performance) {
    if (!performance || !el.catPerformance || !el.catPerformanceStage) return;
    var stageRect = el.catPerformanceStage.getBoundingClientRect();
    var catRect = el.cat.getBoundingClientRect();
    var idleMedia = containedMediaRect(catRect, idleCatSource.width, idleCatSource.height);
    var idleBox = idleCatSource.visibleBox;
    var target = {
      left: idleMedia.left + idleBox[0] / idleCatSource.width * idleMedia.width,
      top: idleMedia.top + idleBox[1] / idleCatSource.height * idleMedia.height,
      width: idleBox[2] / idleCatSource.width * idleMedia.width,
      height: idleBox[3] / idleCatSource.height * idleMedia.height
    };
    var sourceBox = performance.startBox;
    var scale = target.height / sourceBox[3];
    var targetCenterX = target.left + target.width / 2;
    var sourceCenterX = sourceBox[0] + sourceBox[2] / 2;
    var targetBottom = target.top + target.height;
    var sourceBottom = sourceBox[1] + sourceBox[3];
    el.catPerformanceStage.style.setProperty("--video-left", (targetCenterX - stageRect.left - sourceCenterX * scale).toFixed(2) + "px");
    el.catPerformanceStage.style.setProperty("--video-top", (targetBottom - stageRect.top - sourceBottom * scale).toFixed(2) + "px");
    el.catPerformanceStage.style.setProperty("--video-width", (1280 * scale).toFixed(2) + "px");
    el.catPerformanceStage.style.setProperty("--video-height", (720 * scale).toFixed(2) + "px");
    el.tonight.style.setProperty("--video-fade-ms", (performance.transitionMs || 420) + "ms");
  }

  function finishPetVideo(token, actionId) {
    if (token !== actionToken) return;
    el.tonight.dataset.videoExit = "true";
    el.tonight.dataset.videoTransition = "exit";
    el.cat.classList.remove("is-video-playing");
    window.clearTimeout(videoTransitionTimer);
    var exitMs = activeVideoPerformance && activeVideoPerformance.transitionMs || 420;
    videoTailTimer = window.setTimeout(function () {
      if (token !== actionToken) return;
      if (actionId === "capture" && captureTimer) releaseCapture(false);
      else finishPetAction(token);
    }, exitMs);
  }
  ...
  function playPetVideo(actionId, token) {
    ...
    function revealVideo() {
      ...
      alignPetVideo(performance);
      el.tonight.removeAttribute("data-video-exit");
      el.tonight.dataset.videoReady = "true";
      el.tonight.dataset.videoTransition = "enter";
      el.cat.classList.add("is-video-playing");
      window.clearTimeout(videoTransitionTimer);
      videoTransitionTimer = window.setTimeout(function () {
        if (token === actionToken && el.tonight.dataset.videoTransition === "enter") {
          el.tonight.removeAttribute("data-video-transition");
        }
      }, performance.transitionMs || 420);
      var mediaDuration = Number.isFinite(video.duration) ? video.duration * 1000 : performance.duration;
      activeReactionTimer = window.setTimeout(function () {
        if (token === actionToken) finishPetVideo(token, actionId);
      }, mediaDuration + (performance.holdFor || 0) + 1600);
    }
    function fallBackToCodeMotion() {
      if (token !== actionToken) return;
      stopPetVideo();
      activeReactionTimer = window.setTimeout(function () {
        finishPetAction(token);
      }, petActions[actionId].duration);
    }
  ...
  function playPetAction(id, options) {
    ...
    var videoOwnsTiming = false;
    if (performance && !motionQuery.matches) {
      videoOwnsTiming = playPetVideo(id, token);
    }
    if (motionQuery.matches) duration = Math.min(duration, 720);
    if (!videoOwnsTiming) {
      activeReactionTimer = window.setTimeout(function () {
        if (id === "capture" && captureTimer) releaseCapture(false);
        else finishPetAction(token);
      }, duration);
    }
    return true;
  ...
  function playAmbient() {
    ...
    var rareFlicker = recentReactions[recentReactions.length - 1] !== "glitch" && Math.random() < .12;
    var id = rareFlicker ? "glitch" : choices[Math.floor(Math.random() * choices.length)];
